...
static int s3c_fb_remove(struct platform_device *pdev)
{
	...
	pm_runtime_get_sync(sfb->dev);
	devm_pinctrl_put(lcd_io_cfg.pctrl);
	iounmap(lcd_io_cfg.lcd_blk_cfg_reg);
...
static int s3c_fb_suspend(struct device *dev)
{
	...
	printk(KERN_WARNING"in s3c_fb_suspend!!\n");
	pm_runtime_get_sync(sfb->dev);
...
static int s3c_fb_resume(struct device *dev)
{
	...
	printk(KERN_WARNING"in s3c_fb_resume!!\n");
	pm_runtime_get_sync(sfb->dev);
...
static int s3c_fb_runtime_suspend(struct device *dev)
{
	...
	printk(KERN_WARNING"in s3c_fb_runtime_suspend!!\n");
	if (!sfb->variant.has_clksel)
		clk_disable_unprepare(sfb->lcd_clk);
...
static int s3c_fb_runtime_resume(struct device *dev)
{
	...
	printk(KERN_WARNING"in s3c_fb_runtime_resume!!\n");
	clk_prepare_enable(sfb->bus_clk);
